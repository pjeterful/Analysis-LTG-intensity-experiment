%If ctrl and LTG files do not have the same number of stimulus intensities
%use this function intead of spike_analysis_LTG.
...
function [analysis_2_ctrl, analysis_2_LTG]=spike_analysis_LTG2 (ctrl_folder, LTG_folder, spike_window)
...
% Save laser intensities of the stimulations laser
% Do this for each sweep (new laser intensity).
    for sweep=1:size(LTG_exper,2); 
        analysis_2_LTG{fil-2}(1).laser_intensity(sweep,1)=analysis_LTG{1,sweep}.laser_int_max;
        
% Save the start times of the laser stimulations per frequency in
% analysis_2.laser peaks, if the stimulus intensity is 0,fill it with NaN

    for laser = 1:2:LTG_exper(sweep).nr_laser_stim*2; 
        if analysis_2_LTG{fil-2}(1).laser_intensity(sweep,1)==0;
        %LTG files
...
% Save the times of all detected peaks to data file analysis_2.spike_peaks.
% Do this for every repeat 
    for repeat=1:size(analysis_LTG{1, 1}.spikes,2);
...
% Create a struct  with the same size of analysis_2.laser_peaks,and fill it with 0
        ...
        for stim= 1:size(analysis_2_LTG{fil-2}(sweep).laser_peaks);
            % LTG files
...
for stim=1:size(analysis_2_LTG{fil-2}(sweep).det_peaks{1, repeat},1);
    %For LTG files
